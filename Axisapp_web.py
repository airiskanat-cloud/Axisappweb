...
def create_position(
    product_type: str, profile_system: str, width_mm: float, height_mm: float, qty: int,
    left_mm: float = 0, center_mm: float = 0, right_mm: float = 0, top_mm: float = 0,
    sash_count: int = 0, sash_width_mm: float = 0, sash_height_mm: float = 0,
    glass_type: str = "", toning: bool = False, assembly: bool = False, montage: bool = False,
):
    return {
...
class MaterialCalculator:
    ...
    def calculate(self, positions: list):
        ...
        for row in self.ref1:
            row_product_type = str(get_field(row, "тип изделия", "")).strip()
            row_profile_system = str(get_field(row, "система проф", "")).strip()
            element_type = str(get_field(row, "тип элемент", "")).strip()
            product_name = str(get_field(row, "товар", "")).strip()
            ...
            if norm > 0:
                ship_qty = math.ceil(total_fact / norm)
            ...
            material_rows.append({
                ...
                "Товар": product_name,
                "Фактический расход": round(total_fact, 3),
...
class GlassServiceCalculator:
    ...
    def calculate(self, positions: list, aggregates: dict):
        ...
        glass_price = 0.0
        if glass_row:
            glass_price = safe_float(get_field(glass_row, "стоимость стеклопакета"))
        ...
        if glass_row and toning_enabled:
            toning_flag = normalize_key(get_field(glass_row, "тонировка"))
            if toning_flag == "есть":
                toning_price = safe_float(get_field(glass_row, "стоимость тонировки"))
        if toning_price > 0:
        ...
        if glass_row and assembly_enabled:
            assembly_flag = normalize_key(get_field(glass_row, "сборка"))
            if assembly_flag == "есть":
                assembly_price = safe_float(get_field(glass_row, "стоимость сборки"))
        if assembly_price > 0:
        ...
        if glass_row and montage_enabled:
            montage_price = safe_float(get_field(glass_row, "стоимость монтаж"))
        if montage_price > 0:
...
def calculate_final_totals(material_sum: float, glass_service_sum: float):
...
def build_final_table(material_rows: list, glass_rows: list, totals: dict):
    rows = []
    for r in material_rows:
        rows.append({"Наименование": r["Товар"], "Цена": r["Цена"], "Ед.": "—", "Сумма": r["Сумма"]})
    for r in glass_rows:
        rows.append({"Наименование": r["Наименование"], "Цена": r["Цена"], "Ед.": r["Ед."], "Сумма": r["Сумма"]})
    rows.append({"Наименование": "ИТОГО (база)", "Цена": "", "Ед.": "", "Сумма": totals["base_sum"]})
    rows.append({"Наименование": "Обеспечение 65%", "Цена": "", "Ед.": "", "Сумма": totals["ensure_65"]})
    rows.append({"Наименование": "ИТОГО К ОПЛАТЕ", "Цена": "", "Ед.": "", "Сумма": totals["total_sum"]})
    return rows

def serialize_positions(positions: list) -> str:
    try: return json.dumps(positions, ensure_ascii=False)
    except Exception: return ""

def serialize_final_table(final_rows: list) -> str:
    try: return json.dumps(final_rows, ensure_ascii=False)
    except Exception: return ""

def save_request_to_sheet(gs, user, positions, aggregates, material_sum, glass_sum, totals, final_rows):
    row = [
        now_str(), user, len(positions), round(aggregates["total_area"], 3),
        round(aggregates["total_perimeter"], 3), round(material_sum, 2),
        round(glass_sum, 2), round(totals["base_sum"], 2),
        round(totals["ensure_65"], 2), round(totals["total_sum"], 2),
        serialize_positions(positions), serialize_final_table(final_rows),
    ]
...
def generate_kp_excel(template_path, user, aggregates, final_rows, totals):
    wb = load_workbook(template_path)
    ws = wb.active
    ws["B2"], ws["B3"], ws["B4"] = "Коммерческое предложение", f"Менеджер: {user}", f"Дата: {now_str()}"
    ws["B6"], ws["B7"] = aggregates["total_area"], aggregates["total_perimeter"]
    r = 10
    for row in final_rows:
        ws[f"A{r}"], ws[f"B{r}"], ws[f"C{r}"], ws[f"D{r}"] = row["Наименование"], row.get("Ед.", ""), row.get("Цена", ""), row.get("Сумма", "")
        r += 1
...
def kp_download_block(user, aggregates, final_rows, totals):
    st.subheader("📄 Коммерческое предложение")
    kp_template = st.file_uploader("Загрузите шаблон КП (xlsx)", type=["xlsx"])
    if not kp_template: return
    with NamedTemporaryFile(delete=False, suffix=".xlsx") as f:
        f.write(kp_template.read())
        template_path = f.name
    kp_path = generate_kp_excel(template_path, user, aggregates, final_rows, totals)
    with open(kp_path, "rb") as f:
        st.download_button("⬇️ Скачать коммерческое предложение", f, file_name="Коммерческое_предложение.xlsx")
...
def main():
    ...
    product_type = st.sidebar.selectbox("Тип изделия", ["Окно с откр.", "Окно глух.", "Дверь 2-х створч.", "Дверь 1 створч.", "Фасад"])
    profile_system = st.sidebar.selectbox("Система профиля", ["ALG 2030-63C", "ALG 2030-55C", "ALG 2030-73C", "ALG 2030-45C", "ALG 2030-Slim", "Ruit 50F"])
    glass_types = [row.get("тип стеклопакета") for row in gs.read(SHEET_REF2)]
    glass_type = st.sidebar.selectbox("Тип стеклопакета", glass_types if glass_types else [""])
    toning, assembly, montage = st.sidebar.checkbox("Тонировка"), st.sidebar.checkbox("Сборка"), st.sidebar.checkbox("Монтаж")

    st.header("📦 Позиции")
    pos_count = st.number_input("Количество позиций", min_value=1, step=1, value=1)
    positions = []
    for i in range(int(pos_count)):
        st.subheader(f"Позиция #{i+1}")
        c1, c2, c3 = st.columns(3)
        width, height, qty = c1.number_input("Ширина, мм", key=f"w_{i}", step=10.0), c2.number_input("Высота, мм", key=f"h_{i}", step=10.0), c3.number_input("Кол-во", key=f"q_{i}", min_value=1, step=1)
        st.markdown("**Импосты**")
        i1, i2, i3, i4 = st.columns(4)
        left, center, right, top = i1.number_input("LEFT", key=f"l_{i}"), i2.number_input("CENTER", key=f"c_{i}"), i3.number_input("RIGHT", key=f"r_{i}"), i4.number_input("TOP", key=f"t_{i}")
        sash_count, sash_width, sash_height = 0, 0.0, 0.0
        if "Окно с откр." in product_type or "Дверь" in product_type:
            sash_count = st.number_input("Кол-во створок", key=f"sash_n_{i}", min_value=1, step=1)
            s1, s2 = st.columns(2)
            sash_width, sash_height = s1.number_input("Ширина створки, мм", key=f"sash_w_{i}"), s2.number_input("Высота створки, мм", key=f"sash_h_{i}")
        positions.append(create_position(product_type, profile_system, width, height, qty, left, center, right, top, sash_count, sash_width, sash_height, glass_type, toning, assembly, montage))
    ...
    if st.button("🚀 Рассчитать", type="primary"):
        ...
        glass_rows, glass_sum = glass_calc.calculate(positions, aggregates)
        totals = calculate_final_totals(material_sum, glass_sum)
        final_rows = build_final_table(material_rows, glass_rows, totals)
        save_request_to_sheet(gs, st.session_state.get("user", ""), positions, aggregates, material_sum, glass_sum, totals, final_rows)
        st.success(f"ИТОГО К ОПЛАТЕ: {totals['total_sum']}")
        tab1, tab2, tab3 = st.tabs(["📐 Габариты", "🧱 Материалы", "💰 Итог"])
        with tab1:
        ...
        kp_download_block(st.session_state.get("user", ""), aggregates, final_rows, totals)
    st.markdown("---")
    show_history(gs)
